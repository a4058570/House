...
@property(nonatomic,strong)NSMutableArray *seprateLines;
@end
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _seprateLines=[NSMutableArray array];
        self.frame = frame;
...
- (void)setDataSource:(id<LSSelectMenuViewDataSource>)dataSource{
    ...
    [self.topBtns removeAllObjects];
    for(UIView *line in self.seprateLines)
    {
        [line removeFromSuperview];
    }
    [self.seprateLines removeAllObjects];
    ...
    for (int i = 0; i<itemCount; i++) {
        ...
        UIView *lines=[[UIView alloc]initWithFrame:CGRectMake(ww*(i+1), 0, 0.5, hh)];
        [lines setBackgroundColor:[UIColor lightGrayColor]];
        [self addSubview:lines];
        [self.seprateLines addObject:lines];
...
- (void)showTListViewWIthSelectItemView:(LSButton*)sender{
    ...
    BOOL should=YES;
    if ([self.delegate respondsToSelector:@selector(shouldShowMenuView:index:)]) {
        should=[self.delegate shouldShowMenuView:self index:sender.tag-100];
    }
    if (should) {
        ...
        isShow = YES;
        if ([_delegate respondsToSelector:@selector(selectMenuView:didSelectAtIndex:)]) {
...
- (void)removeTListViewWithSelectItemView:(LSButton*)sender{
    ...
    [UIView animateWithDuration:DurationTime animations:^{
        sender.markImgView.transform = CGAffineTransformRotate(sender.markImgView.transform, M_PI);
     //   sender.markImgView.transform = CGAffineTransformMakeRotation( 0);
    } completion:^(BOOL ok){
        if (ok) {
            self.userInteractionEnabled = YES;
    ...
    isShow = NO;
    if ([_delegate respondsToSelector:@selector(selectMenuView:didRemoveAtIndex:)]) {
...
- (void)closeCurrViewWithIndex:(NSInteger)index{
    ...
    [UIView animateWithDuration:DurationTime animations:^{
      //  btn.markImgView.transform = CGAffineTransformRotate(btn.markImgView.transform, M_PI);
        btn.markImgView.transform = CGAffineTransformMakeRotation( 0);
    } completion:^(BOOL ok){
        if (ok) {
            self.userInteractionEnabled = YES;
...
{
   LSButton *btn=(id) [self.topBtns objectAtIndex:index];
    [btn setTitle:title];
  
}
- (void)btnLoadAnim:(BOOL)anim withIndex:(NSInteger)index
{
    LSButton *btn=(id) [self.topBtns objectAtIndex:index];
    if (anim) {
        [btn startLoadingAnim];
    }else
    {
        [btn stopLoadingAnim];
    }
