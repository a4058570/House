...
//            UIView *v= [self.itemViews.allValues firstObject];
//            NSLog(@"")
            if (_vertical)
            {
                ...
                if (offset > 0.0)
                {
                    transform = CATransform3DTranslate(transform, 0.0f, 20.0f, 0.0f);
                    
                    return CATransform3DRotate(transform, -2.5 * offset, 1.0f, 0.0f, 0.0f);
                }
                
                return CATransform3DTranslate(transform, 0.0f, offset * _itemWidth * tilt+20, offset * _itemWidth * spacing);
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    view.superview.layer.anchorPoint = CGPointMake(0.5f, 1.0f);
#else
    
    //center view
    [view.superview setFrameOrigin:NSMakePoint(self.bounds.size.width/2.0 + _contentOffset.width,
                                               self.bounds.size.height/2.0 + _contentOffset.height)];
    if (self.type!=iCarouselTypeTimeMachine) {
        view.superview.layer.anchorPoint = CGPointMake(0.5, 1.0);
    }else
    {
        view.superview.layer.anchorPoint = CGPointMake(0.5, 1.0);
    }
