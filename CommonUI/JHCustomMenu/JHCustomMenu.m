...
- (instancetype)initWithDataArr:(NSArray *)dataArr origin:(CGPoint)origin width:(CGFloat)width rowHeight:(CGFloat)rowHeight direction:(DirectType)orientation
{
    ...
    if (self) {
        ...
        if (orientation == Direct_Left) {
            self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x -width - RightToView, origin.y + TopToView, width, rowHeight * dataArr.count) style:UITableViewStylePlain];
        }else if(orientation == Direct_Right){
...
//    [colors[serie] setFill];
    ...
    if (self.orientation == Direct_Left) {
        
        CGContextMoveToPoint(context,kScreenWidth-RightToView * 2.5, TopToView * 0.5);//设置起点
        
        CGContextAddLineToPoint(context,kScreenWidth-RightToView * 2, TopToView);
        
        CGContextAddLineToPoint(context,kScreenWidth-RightToView * 3, TopToView);
        
    }else if (self.orientation == Direct_Right){
