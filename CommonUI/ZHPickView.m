...
-(void)remove{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        targetFrame.origin=CGPointMake(0, [UIScreen mainScreen].bounds.size.height);
...
-(void)showInView:(UIView *)view{
    ...
    if (self.blackView==nil) {
        self.blackView=[[UIView alloc]initWithFrame:view.bounds];
       
    }
     self.blackView.backgroundColor=[UIColor colorWithRed:0 green:0 blue:0 alpha:0.0];
    ...
    oldFrame.origin=CGPointMake(0, [UIScreen mainScreen].bounds.size.height);
    self.frame=oldFrame;
...
-(void)doneClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString:timesCap:)]) {
        [self.delegate toobarDonBtnHaveClick:self resultString:_resultString timesCap:_datePicker.date.timeIntervalSince1970];
    }
    [self remove];
}
/**
